...
public abstract class DiscoveryRequestStreamObserver<T, U> implements StreamObserver<T> {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(io.envoyproxy.controlplane.server.DiscoveryServer.class);

  final long streamId;
  final io.envoyproxy.controlplane.server.DiscoveryServer<T, U, ?, ?, ?> discoveryServer;
  ...
  public void onNext(T rawRequest) {
    ...
    io.envoyproxy.controlplane.server.LatestDiscoveryResponse latestDiscoveryResponse = latestResponse(requestTypeUrl);
  ...
  private void send(Response response, String typeUrl) {
    ...
    setLatestResponse(
        typeUrl,
        io.envoyproxy.controlplane.server.LatestDiscoveryResponse.create(
            nonce,
  ...
  abstract io.envoyproxy.controlplane.server.LatestDiscoveryResponse latestResponse(String typeUrl);
