...
public class V3DiscoveryServer extends DiscoveryServer<DiscoveryRequest, DiscoveryResponse, DeltaDiscoveryRequest,
  ...
  public V3DiscoveryServer(ConfigWatcher configWatcher, MeterRegistry meterRegistry) {
    this(Collections.emptyList(), configWatcher, meterRegistry);
  }

  public V3DiscoveryServer(io.envoyproxy.controlplane.server.DiscoveryServerCallbacks callbacks,
                           ConfigWatcher configWatcher, MeterRegistry meterRegistry) {
    this(Collections.singletonList(callbacks), configWatcher, meterRegistry);
  }

  public V3DiscoveryServer(
      List<io.envoyproxy.controlplane.server.DiscoveryServerCallbacks> callbacks,
      ConfigWatcher configWatcher, MeterRegistry meterRegistry) {
    this(callbacks, configWatcher, new DefaultExecutorGroup(),
        new DefaultProtoResourcesSerializer(), meterRegistry);
  }

  public V3DiscoveryServer(List<io.envoyproxy.controlplane.server.DiscoveryServerCallbacks> callbacks,
      ConfigWatcher configWatcher,
      io.envoyproxy.controlplane.server.ExecutorGroup executorGroup,
      ProtoResourcesSerializer protoResourcesSerializer,
                           MeterRegistry meterRegistry) {
    this(callbacks, configWatcher, executorGroup, protoResourcesSerializer, new DefaultStartupConfigs(), meterRegistry);
  }
  ...
  public V3DiscoveryServer(List<io.envoyproxy.controlplane.server.DiscoveryServerCallbacks> callbacks,
                           ...
                           MeterRegistry meterRegistry) {
    super(callbacks, configWatcher, executorGroup, protoResourcesSerializer, startupConfigs, meterRegistry);
  }
  ...
  protected void runStreamRequestCallbacks(long streamId, DiscoveryRequest discoveryRequest) throws RequestException {
    for (io.envoyproxy.controlplane.server.DiscoveryServerCallbacks cb : callbacks) {
