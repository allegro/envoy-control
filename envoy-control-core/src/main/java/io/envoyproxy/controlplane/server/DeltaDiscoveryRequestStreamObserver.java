...
public abstract class DeltaDiscoveryRequestStreamObserver<V, X, Y> implements StreamObserver<V> {
  ...
  DeltaDiscoveryRequestStreamObserver(String defaultTypeUrl,
    ...
    this.hasClusterChanged = false;
      ...
      this.pushTimeEds = Timer.builder("envoy_control_send_delta")
      ...
      this.pushTimeRds = Timer.builder("envoy_control_send_delta")
          .serviceLevelObjectives(
              Duration.ofMillis(10),
              Duration.ofMillis(100),
              Duration.ofSeconds(1),
      ...
      this.pushTimeCds = Timer.builder("envoy_control_send_delta")
          .serviceLevelObjectives(
              Duration.ofMillis(10),
              Duration.ofMillis(100),
              Duration.ofSeconds(1),
      ...
      this.pushTimeLds = Timer.builder("envoy_control_send_delta")
          .serviceLevelObjectives(
              Duration.ofMillis(10),
              Duration.ofMillis(100),
              Duration.ofSeconds(1),
          ...
          .tags("type", "lds")
          .register(meterRegistry);
      this.pushTimeUnknown = Timer.builder("envoy_control_send_delta")
          .serviceLevelObjectives(
              Duration.ofMillis(10),
              Duration.ofMillis(100),
              Duration.ofSeconds(1),
    ...
    private Timer getTimer(String typeUrl) {
        return switch (TYPE_URLS_TO_RESOURCE_TYPE.get(typeUrl)) {
  ...
  public void onNext(V rawRequest) {
    ...
    if (!request.getResponseNonce().isEmpty()) {
      ...
      io.envoyproxy.controlplane.server.LatestDeltaDiscoveryResponse response = clearResponse(requestTypeUrl, request.getResponseNonce());
  ...
  private void send(DeltaResponse response, String typeUrl) {
      getTimer(typeUrl).record(() -> {
          ...
          setResponse(
              typeUrl,
              nonce,
              io.envoyproxy.controlplane.server.LatestDeltaDiscoveryResponse.create(
                  nonce,
                  response.version(),
                  response.resources()
                      .entrySet()
  ...
  abstract void setResponse(String typeUrl, String nonce, io.envoyproxy.controlplane.server.LatestDeltaDiscoveryResponse response);
