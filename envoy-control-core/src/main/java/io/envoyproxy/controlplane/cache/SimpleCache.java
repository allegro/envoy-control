 ...
 * {@link io.envoyproxy.controlplane.cache.Snapshot} per node group. For the protocol to work correctly in ADS mode, EDS/RDS requests are responded to
...
public abstract class SimpleCache<T, U extends Snapshot> implements SnapshotCache<T, U> {
  ...
  private final io.envoyproxy.controlplane.cache.NodeGroup<T> groups;
  ...
  public io.envoyproxy.controlplane.cache.Watch createWatch(
      boolean ads,
      XdsRequest request,
  ...
  public io.envoyproxy.controlplane.cache.Watch createWatch(
    ...
    readLock.lock();
    try {
      io.envoyproxy.controlplane.cache.CacheStatusInfo<T> status = statuses.getOrAddStatusInfo(group, requestResourceType);
      ...
      io.envoyproxy.controlplane.cache.Watch watch = new io.envoyproxy.controlplane.cache.Watch(ads, allowDefaultEmptyEdsUpdate, request, responseConsumer);
   ...
   */
  @Override
  public io.envoyproxy.controlplane.cache.DeltaWatch createDeltaWatch(
      DeltaXdsRequest request,
      String requesterVersion,
      Map<String, String> resourceVersions,
      Set<String> pendingResources,
      boolean isWildcard,
      Consumer<io.envoyproxy.controlplane.cache.DeltaResponse> responseConsumer,
      boolean hasClusterChanged) {
    ...
    readLock.lock();
    try {
      io.envoyproxy.controlplane.cache.DeltaCacheStatusInfo<T> status = statuses.getOrAddDeltaStatusInfo(group, requestResourceType);
      status.setLastWatchRequestTime(System.currentTimeMillis());

      U snapshot = snapshots.get(group);
      String version = snapshot == null ? "" : snapshot.version(requestResourceType, Collections.emptyList());

      io.envoyproxy.controlplane.cache.DeltaWatch watch = new io.envoyproxy.controlplane.cache.DeltaWatch(request,
      ...
      if (version.equals(requesterVersion)) {
        ...
        if (!isWildcard && watch.pendingResources().size() != 0) {
          ...
          // version comparisons.
          Map<String, io.envoyproxy.controlplane.cache.VersionedResource<?>> resources = snapshot.versionedResources(request.getResourceType());
          Map<String, io.envoyproxy.controlplane.cache.VersionedResource<?>> requestedResources = watch.pendingResources()
              .stream()
  ...
  public synchronized void setSnapshot(T group, U snapshot) {
    ...
    Map<ResourceType, io.envoyproxy.controlplane.cache.CacheStatusInfo<T>> status;
    Map<ResourceType, io.envoyproxy.controlplane.cache.DeltaCacheStatusInfo<T>> deltaStatus;
    U previousSnapshot;
    writeLock.lock();
    try {
  ...
  public StatusInfo<T> statusInfo(T group) {
    ...
    try {
      Map<ResourceType, io.envoyproxy.controlplane.cache.CacheStatusInfo<T>> statusMap = statuses.getStatus(group);
      Map<ResourceType, io.envoyproxy.controlplane.cache.DeltaCacheStatusInfo<T>> deltaStatusMap = statuses.getDeltaStatus(group);
  ...
  protected void respondWithSpecificOrder(T group,
                                          ...
                                          Map<ResourceType, io.envoyproxy.controlplane.cache.CacheStatusInfo<T>> statusMap,
                                          Map<ResourceType, io.envoyproxy.controlplane.cache.DeltaCacheStatusInfo<T>> deltaStatusMap) {
    for (ResourceType resourceType : RESOURCE_TYPES_IN_ORDER) {
      ...
      if (deltaStatus != null) {
        Map<String, io.envoyproxy.controlplane.cache.VersionedResource<?>> previousResources = previousSnapshot == null
            ? Collections.emptyMap()
            : previousSnapshot.versionedResources(resourceType);
        Map<String, io.envoyproxy.controlplane.cache.VersionedResource<?>> snapshotResources = snapshot.versionedResources(resourceType);

        Map<String, io.envoyproxy.controlplane.cache.VersionedResource<?>> snapshotChangedResources = snapshotResources.entrySet()
            .stream()
            .filter(entry -> {
              io.envoyproxy.controlplane.cache.VersionedResource<?> versionedResource = previousResources.get(entry.getKey());
        ...
        deltaStatus.watchesRemoveIf((id, watch) -> {
          ...
          if (!watch.version().equals(version)) {
            ...
            Map<String, io.envoyproxy.controlplane.cache.VersionedResource<?>> changedResources = findChangedResources(watch, snapshotChangedResources);
  ...
  private Response createResponse(XdsRequest request, Map<String, io.envoyproxy.controlplane.cache.VersionedResource<?>> resources,
                                  String version, boolean allowDefaultResource) {
    Collection<? extends Message> filtered = request.getResourceNamesList().isEmpty()
        ? resources.values().stream()
        .map(io.envoyproxy.controlplane.cache.VersionedResource::resource)
        .collect(Collectors.toList())
  ...
  private boolean respond(Watch watch, U snapshot, T group) {
    Map<String, io.envoyproxy.controlplane.cache.VersionedResource<?>> snapshotResources =
  ...
  private List<String> findRemovedResources(io.envoyproxy.controlplane.cache.DeltaWatch watch, Map<String, io.envoyproxy.controlplane.cache.VersionedResource<?>> snapshotResources) {
  ...
  private Map<String, io.envoyproxy.controlplane.cache.VersionedResource<?>> findChangedResources(io.envoyproxy.controlplane.cache.DeltaWatch watch,
                                                                                                  Map<String, io.envoyproxy.controlplane.cache.VersionedResource<?>> snapshotResources) {
  ...
  private ResponseState respondDelta(DeltaXdsRequest request, io.envoyproxy.controlplane.cache.DeltaWatch watch, U snapshot, String version, T group) {
    Map<String, io.envoyproxy.controlplane.cache.VersionedResource<?>> snapshotResources = snapshot.versionedResources(request.getResourceType());
    List<String> removedResources = findRemovedResources(watch,
        snapshotResources);
    Map<String, io.envoyproxy.controlplane.cache.VersionedResource<?>> changedResources = findChangedResources(watch, snapshotResources);
  ...
  private ResponseState respondDelta(DeltaWatch watch,
    ...
    io.envoyproxy.controlplane.cache.DeltaResponse response = DeltaResponse.create(
        watch.request(),
        resources,
        removedResources,
        version);
  ...
  private io.envoyproxy.controlplane.cache.VersionedResource<?> defaultResource(String resourceName, ResourceType resourceType) {
