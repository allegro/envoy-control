...
class NodeMetadataValidator(
    val outgoingPermissionsProperties: OutgoingPermissionsProperties
    ...
    private fun validateDependencies(metadata: NodeMetadata) {
        if (!outgoingPermissionsProperties.enabled) {
            return
        }
    ...
    private fun hasAllServicesDependencies(metadata: NodeMetadata) =
        metadata.proxySettings.outgoing.containsDependencyForService(
            outgoingPermissionsProperties.allServicesDependenciesValue
        )

    private fun isAllowedToHaveAllServiceDependencies(metadata: NodeMetadata) = outgoingPermissionsProperties
        .servicesAllowedToUseWildcard.contains(metadata.serviceName)
}
